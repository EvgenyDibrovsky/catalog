        ...
        <div className="bg-white dark:bg-darkBgContent flex flex-col gap-2 p-5 mb-4">
          {logo && (
            <img
              ...
              alt={name_company}
              className="h-10 object-contain mb-2 sm:mb-2 mr-auto"
            />
          )}

          {name_company && (
            <p className="text-black dark:text-white font-bold text-[1.1rem] mb-1">
              {name_company}
            </p>
          )}
          ...
          {nip && (
            <div className="flex gap-2">
              <p className="text-black dark:text-white font-bold w-3/12">
                {t('pages_category.developers.page_item.nip')}:
              </p>
          ...
          {link_site && (
            <div className="flex gap-2">
              <p className="text-black dark:text-white font-bold w-3/12">
                {t('pages_category.developers.page_item.www')}:
              </p>
              <a
                href={`https://${link_site}`}
                target="_blank"
                rel="noreferrer"
                className="text-black dark:text-white hover:text-sky-500 dark:hover:text-yellow-500 hover:underline">
                {link_site}
              </a>
            </div>
          )}
          {address && (
            <div className="flex gap-2">
              <p className="text-black dark:text-white font-bold w-3/12">
                {t('pages_category.developers.page_item.address')}:
              </p>
          ...
          {(nr_tel_1 || nr_tel_2) && (
            <div className="flex gap-2">
              <p className="text-black dark:text-white font-bold w-3/12">
                {t('pages_category.developers.page_item.tel')}:
              </p>
          ...
          {(email_1 || email_2) && (
            <div className="flex gap-2">
              <p className="text-black dark:text-white font-bold w-3/12">
                {t('pages_category.developers.page_item.email')}:
              </p>
              <div className="flex flex-col">
                {email_1 && (
                  <a
